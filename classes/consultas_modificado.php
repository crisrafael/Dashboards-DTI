...
class Consulta {
    ...
    public function slaPorFila() {
        ...
        and t.change_time between '2016-08-01 00:01' and '2016-08-31 23:59'
        and q.id not in (22,24,25,27,28,29)
		and q.valid_id = 1
        GROUP BY q.name
        ORDER BY fila desc";
    ...
    public function encInstituicaoTresMeses ($instituicao){
        
        $query = "select DATE_FORMAT(t.change_time, '%m') as 'mes',
                    DATE_FORMAT(t.change_time, '%Y') as 'ano',	
                    count(t.tn) as 'total'
                    from ticket t
                    join dynamic_field_value dfv on (t.id = dfv.object_id)
                    where t.ticket_state_id in (2,3,10)
                    and dfv.field_id = 17
                    and dfv.value_text like '$instituicao'
                    -- and YEAR(t.change_time) = YEAR(now())
                    and t.change_time between curdate() - interval 3 month and curdate()
                    group by DATE_FORMAT(t.change_time, '%m%Y')";
        ...
        while ($linha = mysql_fetch_array($result)) {
            ...
            $ret["total"] .= "{x: new Date(" .$linha["ano"].= "," . $linha["mes"] . "), y: " . $linha["total"] . "}";                      
    ...
    public function getFilas() {
        $query = "SELECT * from queue
                    where id not in (22,24,25,27,28,29)
                    order by name";
